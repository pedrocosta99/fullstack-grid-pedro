...
export default function SheetPage() {
  ...
  const [selectedCell, setSelectedCell] = useState<CellAddress | null>(toCellAddress('A1'));
  ...
  const handleCellUpdate = useCallback(async (address: CellAddress, value: string) => {
    if (!sheet) return;
    ...
    try {
      ...
      const edit = isEmpty
        ...
        : { addr: address, kind: 'literal' as const, value: isNaN(Number(value)) ? value : Number(value) };
  ...
  const handleExtendSheet = useCallback(async (direction: 'row' | 'col') => {
    if (!sheet) return;
  ...
  const handleSort = useCallback(async (column: number, ascending: boolean) => {
    if (!sheet) return;
      ...
      <div className="bg-white/90 backdrop-blur-sm border-b border-neutral-200/60 px-6 py-5 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 bg-gradient-to-br from-primary-500 to-accent-500 rounded-xl flex items-center justify-center shadow-liquid-purple">
            ...
            <h1 className="text-2xl font-bold text-neutral-900">{sheet.name}</h1>
          </div>
